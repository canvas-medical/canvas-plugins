...
from recurring_appointments.utils.constants import RecurrenceEnum, STATES, STATES_DICT, FIELD_STATE_KEY, FIELD_RECURRENCE_KEY
...
class AppointmentFormFields(BaseHandler):
    ...
    def compute(self) -> list[Effect]:
        ...
        return [
            AppointmentsMetadataCreateFormEffect(
                form_fields=[
                  recurrence_form_field, state_form_field
                ]
            ).apply()
        ]
...
class AppointmentProviderFormField(BaseHandler):
    ...
    def compute(self) -> list[Effect]:
        ...
        state_selected = STATES_DICT[state_field["values"]] if state_field else None

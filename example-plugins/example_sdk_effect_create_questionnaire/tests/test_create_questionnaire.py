...
def test_a_questionnaire_the_schema_rejects_is_reported(route: CreateQuestionnaireAPI) -> None:
    """A title too long for its column is answered with the reason, not an empty 500.

    The effect raises on apply(), and that error reaches the same handler as the route's own
    rejections, so the caller learns what was wrong either way.
    """
    route.request = DummyRequest(
        {"title": "x" * 242, "code": "1", "items": [{"prompt": "p", "choices": ["a"]}]}
    )

    assert "too long" in error_from(route)


def test_no_questions_is_reported(route: CreateQuestionnaireAPI) -> None:
    """A definition with no items is rejected, and the message names the field."""
    route.request = DummyRequest({"title": "Empty", "code": "1", "items": []})

    assert "questions: [] should be non-empty" in error_from(route)

...
`CreateQuestionnaire` validates the configuration when the effect is applied, so a malformed
questionnaire raises inside the plugin. Failures that happen on the Canvas side after that are
reported to Sentry and are not visible to the plugin.

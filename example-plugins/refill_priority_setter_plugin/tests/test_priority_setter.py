...
from refill_priority_setter_plugin.handlers.priority_setter import RefillTaskPriorityHandler
...
def test_priority_setter_protocol_configuration() -> None:
    ...
    assert EventType.Name(EventType.TASK_CREATED) in RefillTaskPriorityHandler.RESPONDS_TO
    assert EventType.Name(EventType.TASK_UPDATED) in RefillTaskPriorityHandler.RESPONDS_TO
    assert (
        EventType.Name(EventType.TASK_COMMAND__POST_UPDATE) in RefillTaskPriorityHandler.RESPONDS_TO
    )


def test_priority_setter_protocol_imports() -> None:
    """Test that the protocol can be imported and has expected attributes."""
    assert RefillTaskPriorityHandler is not None
    assert hasattr(RefillTaskPriorityHandler, "RESPONDS_TO")
    assert hasattr(RefillTaskPriorityHandler, "compute")
...
def test_priority_setter_sets_urgent_for_refill_team() -> None:
    ...
    protocol = RefillTaskPriorityHandler(event=mock_event)
    ...
    with (
        patch(
            "refill_priority_setter_plugin.handlers.priority_setter.Task.objects.get",
            return_value=mock_task,
        ),
        patch(
            "refill_priority_setter_plugin.handlers.priority_setter.UpdateTask"
        ) as mock_update_task_class,
        patch(
            "refill_priority_setter_plugin.handlers.priority_setter.AddTask"
        ) as mock_add_task_class,
        patch("refill_priority_setter_plugin.handlers.priority_setter.log"),
    ):
        ...
        mock_update_task_class.assert_called_once_with(id="task-123", priority=TaskPriority.URGENT)
...
def test_priority_setter_skips_closed_tasks() -> None:
    ...
    with patch(
        ...
        with patch("refill_priority_setter_plugin.handlers.priority_setter.log"):
            effects = protocol.compute()
...
def test_priority_setter_command_event_type() -> None:
    ...
    test_context = {"fields": {"assign_to": {"value": "team-456"}, "priority": None}}

    mock_team = Mock()
    mock_team.responsibilities = [TeamResponsibility.PROCESS_REFILL_REQUESTS]

    with (
        patch.object(type(protocol), "context", property(lambda self: test_context)),
        patch(
            "refill_priority_setter_plugin.handlers.priority_setter.Team.objects.get",
            return_value=mock_team,
        ),
        patch(
            "refill_priority_setter_plugin.handlers.priority_setter.TaskCommand"
        ) as mock_task_command_class,
        patch("refill_priority_setter_plugin.handlers.priority_setter.log"),
    ):
        ...
        mock_task_command_class.assert_called_once_with(
            command_uuid="task-command-123", priority=TaskPriority.URGENT
        )

...
class SearchFullscriptSupplements(BaseProtocol):
    ...
    def compute(self):
        ...
        if not user_id:
            ...
            return [Effect(type=EffectType.AUTOCOMPLETE_SEARCH_RESULTS, payload=json.dumps(results))]
        ...
        result = FullscriptAPI.fetch_products(access_token, query, self.secrets["FULLSCRIPT_SEARCH_PAGE_SIZE"])
        ...
        for product in fullscript_products:
            ...
            post_processed_results.insert(0,{
                "text": name,
                "annotations": ["Supp 🌱"],
                "value": code,
                "extra": {

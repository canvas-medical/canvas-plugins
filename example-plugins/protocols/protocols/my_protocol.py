...
from canvas_sdk.v1.data.compound_medication import CompoundMedication as CompoundMedicationModel, CompoundMedication
...
class HandleThree(BaseHandler):
    ...
    def compute(self) -> list[Effect]:
        ...
        if cmd.note.patient.id == "7c81b0768ccf4938be3df1d5c7a97369" and cmd.state == "in_review":
            actions = [action for action in actions if action["name"] == "sign_action" or action["name"] == "make_changes"]

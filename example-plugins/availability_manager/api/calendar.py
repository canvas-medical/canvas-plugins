...
class CalendarAPI(StaffSessionAuthMixin, SimpleAPIRoute):
    ...
    def post(self) -> list[Response | Effect]:
        ...
        calendar_id = Calendar.objects.for_calendar_name(provider_name=provider_name, calendar_type=calendar_type, location=location_name if location_name else None).values_list("id", flat=True).last()
        ...
        if calendar_id:
            ...
            create_calendar = CreateCalendar(id=calendar_id, provider=provider, type=calendar_type, location=location if location else None, description=description)

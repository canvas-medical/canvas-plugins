...
class TestSupervisingProviderPrescribeProtocolCard:
    """Test suite for supervising_provider_prescribe_protocol Handler."""

    def test_responds_to_correct_event(self) -> None:
        """Test that Handler responds to NOTE_STATE_CHANGE_EVENT_CREATED event."""
        from supervising_provider_prescribe_protocol.handlers.my_handler import Handler

        assert EventType.Name(EventType.NOTE_STATE_CHANGE_EVENT_CREATED) == Handler.RESPONDS_TO
    ...
    def test_compute_with_staff_available(self, monkeypatch: MonkeyPatch) -> None:
        ...
        with patch(
            "supervising_provider_prescribe_protocol.handlers.my_handler.Staff.objects.first"
        ) as mock_staff_first:
            mock_staff_first.return_value = mock_staff

            # Mock PrescribeCommand
            with patch(
                "supervising_provider_prescribe_protocol.handlers.my_handler.PrescribeCommand"
                ...
                # Mock ProtocolCard
                with patch(
                    "supervising_provider_prescribe_protocol.handlers.my_handler.ProtocolCard"
                ) as mock_card_class:

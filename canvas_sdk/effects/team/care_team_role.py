...
class CareTeamRole(TrackableFieldsModel):
    """Effect to create, update, or delete a CareTeamRole (e.g. PCP, Specialist)."""
    ...
    def _get_error_details(self, method: Any) -> list[InitErrorDetails]:
        ...
        if method == "create":
            for required in ("display", "system"):
                if not getattr(self, required):
                    errors.append(
                        self._create_error_detail(
                            "missing",
                            f"Field '{required}' is required to create a care team role.",
        ...
        if method in ("update", "delete") and not self.id:
            errors.append(
                self._create_error_detail(
                    "missing",
                    f"Field 'id' is required to {method} a care team role.",
                    self.id,
                )
            )
        return errors

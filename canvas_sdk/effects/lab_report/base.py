...
A report is identified by a plugin-supplied ``reference_id`` handle (stable
...
class LabReport(TrackableFieldsModel):
    ...
    reference_id: Annotated[str, Field(max_length=40)] | None = None  # plugin-supplied handle
        ...
        if method == "create":
            ...
            if not self.reference_id:
                errors.append(
                    self._create_error_detail(
                        "value",
                        "reference_id is required when creating a lab report (it is the "
                        "plugin's stable handle for later attaching results).",
                        self.reference_id,
                    )
                )

        if method in ("update", "enter_in_error") and not (self.report_id or self.reference_id):
            errors.append(
                self._create_error_detail(
                    "value",
                    "report_id or reference_id is required to reference a lab report.",
                    None,
                )
            )
    ...
    def enter_in_error(self) -> Effect:
        ...
        Only the report handle (``report_id`` or ``reference_id``) is sent;

...
def test_create_and_send_message_with_delay_seconds(
    mock_db_queries: dict[str, MagicMock], valid_message_data: dict[str, Any]
) -> None:
    """Test that create_and_send(delay_seconds=30) sets the field on the Effect."""
    message = Message(**valid_message_data)
    effect = message.create_and_send(delay_seconds=30)
    assert effect.HasField("delay_seconds")
    assert effect.delay_seconds == 30


def test_edit_message_with_delay_seconds(mock_db_queries: dict[str, MagicMock]) -> None:
    """Test that edit(delay_seconds=45) sets the field on the Effect."""
    message = Message(
        message_id=str(uuid4()),
        content="Updated content",
        sender_id=str(uuid4()),
    ...
    effect = message.edit(delay_seconds=45)
    assert effect.HasField("delay_seconds")
    assert effect.delay_seconds == 45


def test_send_message_with_delay_seconds(
    mock_db_queries: dict[str, MagicMock], valid_message_data: dict[str, Any]
) -> None:
    """Test that send(delay_seconds=10) sets the field on the Effect."""
    valid_message_data["message_id"] = str(uuid4())
    message = Message(**valid_message_data)
    effect = message.send(delay_seconds=10)
    assert effect.HasField("delay_seconds")

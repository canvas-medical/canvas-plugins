...
def test_create_appointment_with_delay_seconds(
    mock_db_queries: dict[str, MagicMock], valid_appointment_data: dict[str, Any]
) -> None:
    """Test that create(delay_seconds=60) sets the field on the Effect."""
    appointment = Appointment(**valid_appointment_data)
    effect = appointment.create(delay_seconds=60)
    assert effect.HasField("delay_seconds")
    assert effect.delay_seconds == 60


def test_create_appointment_without_delay_seconds(
    mock_db_queries: dict[str, MagicMock], valid_appointment_data: dict[str, Any]
) -> None:
...
def test_update_appointment_with_delay_seconds(mock_db_queries: dict[str, MagicMock]) -> None:
    """Test that update(delay_seconds=30) sets the field on the Effect."""
    appointment = Appointment(instance_id=str(uuid4()))
    appointment.start_time = datetime.datetime.now()
    effect = appointment.update(delay_seconds=30)
    assert effect.HasField("delay_seconds")
    assert effect.delay_seconds == 30


def test_cancel_appointment_with_delay_seconds(mock_db_queries: dict[str, MagicMock]) -> None:
    """Test that cancel(delay_seconds=45) sets the field on the Effect."""
    appointment = Appointment(instance_id=str(uuid4()))
    appointment.status = AppointmentProgressStatus.CANCELLED

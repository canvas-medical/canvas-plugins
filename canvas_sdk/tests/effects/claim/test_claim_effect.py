...
def mock_db_queries() -> Generator[dict[str, MagicMock]]:
    ...
    with (
        patch("canvas_sdk.effects.claim.claim_comment.Claim.objects") as mock_claim_comment,
        patch("canvas_sdk.effects.claim.claim_label.Claim.objects") as mock_claim_label,
        patch("canvas_sdk.effects.claim.claim_queue.Claim.objects") as mock_claim_queue,
        patch("canvas_sdk.effects.claim.claim_queue.ClaimQueue.objects") as mock_queue,
    ):
        # Setup default behaviors - objects exist
        mock_claim_comment.filter.return_value.exists.return_value = True
        mock_claim_label.filter.return_value.exists.return_value = True
        mock_claim_queue.filter.return_value.exists.return_value = True
...
def test_claim_effect_move_to_queue_requires_existing_claim(
    ...
    mock_db_queries["claim_queue"].filter.return_value.exists.return_value = False

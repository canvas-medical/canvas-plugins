...
def test_create_requires_patient_id() -> None:
    ...
    with pytest.raises(ValidationError):
        LabReport(reference_id="myplugin:ocr-44").create()


def test_create_requires_reference_id() -> None:
    """create() fails without the plugin's reference_id handle."""
    with pytest.raises(ValidationError):
...
def test_create_forbids_report_id() -> None:
    ...
    with pytest.raises(ValidationError):
        LabReport(report_id=REPORT_ID, patient_id="p", reference_id="x").create()


def test_reference_id_max_length_enforced() -> None:
    """reference_id over 40 chars is rejected by pydantic at construction."""
    with pytest.raises(ValidationError):
        LabReport(reference_id="a" * 41, patient_id="p")
...
def test_update_emits_dirty_fields_only() -> None:
    ...
    effect = LabReport(reference_id="myplugin:ocr-44", report_name="Complete Blood Count").update()
...
def test_update_requires_a_handle() -> None:
    """update() fails without report_id or reference_id."""
    with pytest.raises(ValidationError):
...
def test_enter_in_error_emits_handle_only() -> None:
    """enter_in_error() emits ENTER_IN_ERROR_LAB_REPORT with only the reference_id handle."""
    effect = LabReport(reference_id="myplugin:ocr-44").enter_in_error()

    assert effect.type == EffectType.ENTER_IN_ERROR_LAB_REPORT
    assert json.loads(effect.payload) == {"data": {"reference_id": "myplugin:ocr-44"}}
...
def test_enter_in_error_ignores_extra_fields() -> None:
    ...
    effect = LabReport(
        reference_id="myplugin:ocr-44", report_name="ignored", patient_id="p"
    ).enter_in_error()
    assert json.loads(effect.payload) == {"data": {"reference_id": "myplugin:ocr-44"}}

...
def test_create_event_with_delay_seconds() -> None:
    """Event.create(delay_seconds=60) should set the field on the Effect."""
    effect = Event(
        calendar_id="calendar-id",
        title="Appointment",
        starts_at=datetime(2025, 1, 15, 10, 0, 0),
...
def test_create_event_without_delay_seconds() -> None:
    """Event.create() without delay_seconds should not set the field."""
    effect = Event(
        calendar_id="calendar-id",
        title="Appointment",
        starts_at=datetime(2025, 1, 15, 10, 0, 0),
...
def test_update_event_with_delay_seconds() -> None:
    """Event.update(delay_seconds=30) should set the field on the Effect."""
    effect = Event(
        event_id="event-id",
        title="Updated Appointment",
        starts_at=datetime(2025, 1, 15, 11, 0, 0),
        ends_at=datetime(2025, 1, 15, 12, 0, 0),
    ).update(delay_seconds=30)
    assert effect.HasField("delay_seconds")
    assert effect.delay_seconds == 30


def test_delete_event_with_delay_seconds() -> None:
    """Event.delete(delay_seconds=10) should set the field on the Effect."""
    effect = Event(event_id="event-id").delete(delay_seconds=10)
    assert effect.HasField("delay_seconds")

...
from canvas_sdk.effects import questionnaire as questionnaire_effect
...
def test_duplicate_question_coding_is_rejected(config: QuestionnaireConfig) -> None:
    ...
    with pytest.raises(ValidationError, match=r"INTERNAL\|Q1 is used more than once"):
        CreateQuestionnaire(questionnaire=config).apply()
...
def test_duplicate_response_code_is_rejected(config: QuestionnaireConfig) -> None:
    ...
    with pytest.raises(ValidationError, match="Response code Q1A1 is used more than once"):
        CreateQuestionnaire(questionnaire=config).apply()
...
def test_condition_on_unknown_question_is_rejected(config: QuestionnaireConfig) -> None:
    ...
    with pytest.raises(ValidationError, match="NOT_A_QUESTION"):
        CreateQuestionnaire(questionnaire=config).apply()
...
def test_condition_on_unknown_response_is_rejected(config: QuestionnaireConfig) -> None:
    ...
    with pytest.raises(ValidationError, match="NOT_A_RESPONSE"):
        CreateQuestionnaire(questionnaire=config).apply()
...
def test_condition_on_an_ambiguous_question_code_is_rejected(
    ...
    with pytest.raises(ValidationError, match="2 questions use that code"):
        CreateQuestionnaire(questionnaire=config).apply()
...
def test_every_problem_is_reported_at_once(config: QuestionnaireConfig) -> None:
    """One apply() reports every fault, so a caller fixes them in one pass.

    Both faults here are strings of the right type, so they reach the schema. A missing or
    mistyped key is rejected by pydantic on construction instead, before this pass runs.
    """
    config["name"] = "x" * 242
    config["questions"][0]["responses_type"] = "SLIDER"
    ...
    assert "is too long" in messages
    assert "'SLIDER' is not one of" in messages
    assert raised.value.error_count() == 2


def test_the_configuration_is_validated_once_per_apply(
    config: QuestionnaireConfig, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Validation runs a single time.

    The base model sets revalidate_instances="always", so moving these checks into an
    Annotated AfterValidator would run them twice: once on construction and again inside
    apply()'s own model_validate call.
    """
    passes = []
    original = questionnaire_effect.config_errors

    def counting_config_errors(value: QuestionnaireConfig) -> Iterator[str]:
        passes.append(value)
        yield from original(value)

    monkeypatch.setattr(questionnaire_effect, "config_errors", counting_config_errors)

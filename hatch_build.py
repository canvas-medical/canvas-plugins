...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        """
        allowed_modules_file = pathlib.Path(ALLOWED_MODULE_IMPORTS_PATH)

        if not allowed_modules_file.exists():
            raise Exception(
                f"'{ALLOWED_MODULE_IMPORTS_PATH}' does not exist; run the "
                "generation command (`uv run python -m plugin_runner.generate_allowed_modules`)"
            )

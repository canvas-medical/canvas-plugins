...
def get_secret_value(secrets, secret_name: str) -> Optional[str]:
    """
    ...
    try:
        ...
        if isinstance(secrets, dict):
            ...
            if secret_name in secrets and secrets[secret_name]:
                ...
                log.info(f"PDMP-Secrets: Individual '{secret_name}' not found or empty, checking all_secrets fallback")
            
            # PRIORITY 2: Fallback to all_secrets JSON
            all_secrets_json = secrets.get("all_secrets")
            if all_secrets_json:
                log.info(f"PDMP-Secrets: Found all_secrets field with {len(all_secrets_json)} characters")
                try:
                    all_secrets = json.loads(all_secrets_json)
                    log.info(f"PDMP-Secrets: Parsed all_secrets keys: {list(all_secrets.keys())}")
                    if isinstance(all_secrets, dict) and secret_name in all_secrets and all_secrets[secret_name]:
                        value = str(all_secrets[secret_name])
                        log.info(f"PDMP-Secrets: Found in all_secrets '{secret_name}' = '{value}'")
                        return value
                    else:
                        log.warning(f"PDMP-Secrets: '{secret_name}' not found or empty in all_secrets keys: {list(all_secrets.keys())}")
                except json.JSONDecodeError as e:
                    log.error(f"PDMP-Secrets: Failed to parse all_secrets JSON: {e}")
            else:
                log.warning(f"PDMP-Secrets: No all_secrets field found or it's empty")
            
            # Neither individual nor all_secrets contains the secret
            log.error(f"PDMP-Secrets: '{secret_name}' not found in either individual secrets or all_secrets")
        else:
            log.warning(f"PDMP-Secrets: Secrets is not a dict after parsing")
...
def get_secrets_for_environment(
    secrets, use_test_env: bool = False
) -> Dict[str, str]:
    """

...
from pdmp_bamboo.utils.data_extractor import extract_all_data_for_pdmp
from pdmp_bamboo.utils.xml_mapper import create_pdmp_xml
from pdmp_bamboo.utils.xml_request import (
    send_pdmp_request,
    create_pdmp_auth_headers,
    build_pdmp_api_url,
)
from pdmp_bamboo.utils.error_html import create_error_html
from pdmp_bamboo.utils.pdmp_parser import (
    parse_pdmp_response,
    generate_report_button_html,
    generate_scores_html,
    generate_messages_html,
)
from pdmp_bamboo.utils.template_reader import read_template_xml, get_mock_patient_data
...
def handle_pdmp_request(
    ...
    try:
        ...
        modal_effects = []
        if result.get("status") == "success":
            modal_effects = [_create_success_modal(result, use_test_env)]
        else:
...
def _create_success_modal(result: Dict[str, Any], use_test_env: bool = False) -> Effect:
    ...
    if parsed_data.get("parsed") and parsed_data.get("report_url"):
        ...
        report_button_html = generate_report_button_html(report_url, expiration_date, env)
...
def handle_pdmp_mock_request(target: str, context: dict, secrets: dict, event) -> List[Effect]:
    """
    Send PDMP request using mock template data for testing functionality.

    Args:
        target: Patient ID from button target (used for assessment only)
        context: Canvas context dict containing user info
        secrets: Plugin secrets dict
        event: Canvas event object
    ...
    Returns:
        List of Effects (assessment effects + modal effects)
    """
    log.info("PDMP-Workflow: Starting PDMP mock request workflow using template data")

    try:
        # Get patient ID and practitioner ID for assessment creation
        patient_id = target
        practitioner_id = None
        if isinstance(context, dict) and context.get("user"):
            practitioner_id = context["user"].get("id")
        ...
        log.info(f"PDMP-Workflow: Mock request - secrets content: {secrets}")
        
        # Validate secrets and build test API URL
        try:
            base_url = get_secret_value(secrets, "TEST_PDMP_API_URL")
            log.info(f"PDMP-Workflow: Retrieved TEST_PDMP_API_URL: {base_url}")
            if not base_url:
                return [
                    _create_error_modal(
                        "Configuration Error",
                        [
                            "Secret 'TEST_PDMP_API_URL' is required for mock requests but not found in plugin secrets or all_secrets JSON"
                        ],
                    )
                ]
            api_url = build_pdmp_api_url(base_url)
        ...
        # Read mock XML template
        mock_xml = read_template_xml("patient-request.xml")
        if not mock_xml:
            return [
                _create_error_modal(
                    "Template Error", ["Could not read patient-request.xml template file"]
                )
            ]

        # Create auth headers for test environment
        try:
            headers = create_pdmp_auth_headers(secrets, use_test_env=True)
        except ValueError as e:
            return [_create_error_modal("Authentication Error", [str(e)])]

        # Send mock PDMP request
        result = send_pdmp_request(
            api_url=api_url,
        ...
        # Add context
        result["patient_id"] = patient_id
        result["mock_data_used"] = True
        result["extraction_errors"] = []  # No extraction errors for mock data

        # Create structured assessment to document PDMP mock test
        assessment_effects = []
        try:
            if result.get("status") == "success":
                # Only create assessment if PDMP request was successful
        ...
        modal_effects = []
        if result.get("status") == "success":
            modal_effects = [_create_mock_success_modal(result, use_test_env=True)]
        else:
            modal_effects = [_create_mock_error_modal(result)]

        # Return both assessment effects and modal effects
        return assessment_effects + modal_effects

    except Exception as e:
        log.error(f"PDMP-Workflow: Unexpected error in mock request: {str(e)}")
        return [_create_error_modal("Unexpected Error", [str(e)])]


def _create_mock_success_modal(result: Dict[str, Any], use_test_env: bool = True) -> Effect:
    """Create success modal for mock data request."""

    # Parse PDMP response for enhanced display
    raw_response = result.get("raw_response", "")
    parsed_data = parse_pdmp_response(raw_response)

    # Get mock patient data for display
    mock_data = get_mock_patient_data()
    ...
    # Mock data display
    mock_data_html = f"""
    <div style="background-color: #f3e5f5; padding: 15px; border-radius: 4px; margin: 15px 0; border: 1px solid #e1bee7;">
        <h4 style="margin-top: 0; color: #7b1fa2;">🧪 Mock Data Used for Testing</h4>
        <div style="font-size: 14px; color: #4a148c;">
            <p><strong>Mock Patient:</strong> {mock_data["patient"]["first_name"]} {mock_data["patient"]["middle_name"]} {mock_data["patient"]["last_name"]}</p>
            <p><strong>Mock Practitioner:</strong> {mock_data["practitioner"]["first_name"]} {mock_data["practitioner"]["last_name"]} ({mock_data["practitioner"]["role"]})</p>
            <p><strong>Mock Organization:</strong> {mock_data["organization"]["name"]}</p>
        </div>
    </div>
    """
    ...
    html_content = f"""
    <div style="padding: 20px; font-family: Arial, sans-serif;">
        <h3 style="color: #7b1fa2;">🧪 PDMP Mock Request Successful</h3>
        
        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 4px; margin: 15px 0;">
            <p><strong>Status Code:</strong> {result.get("status_code", "N/A")}</p>
            <p><strong>Patient ID:</strong> {result.get("patient_id", "N/A")}</p>
            <p><strong>Environment:</strong> Test Environment (Mock Data)</p>
            {"<p><strong>Request ID:</strong> " + parsed_data.get("request_id", "N/A") + "</p>" if parsed_data.get("request_id") else ""}
        </div>
    ...
    return LaunchModalEffect(
        content=html_content,
        target=LaunchModalEffect.TargetType.DEFAULT_MODAL,
        title="🧪 PDMP Mock Request Successful",
    ).apply()


def _create_mock_error_modal(result: Dict[str, Any]) -> Effect:
    """Create error modal for mock data request."""
    error_list = [
        f"PDMP API Error (Mock Request): {result.get('error', 'Unknown error')}",
        f"Status Code: {result.get('status_code', 'N/A')}",
        "Note: This was a test request using mock template data",
    ]
    ...
    html_content = create_error_html(error_list, "PDMP Mock Request Failed")

    return LaunchModalEffect(
        content=html_content,
        target=LaunchModalEffect.TargetType.DEFAULT_MODAL,
        title="❌ PDMP Mock Request Failed",
    ).apply()

...
class BaseXMLBuilder(ABC):
    ...
    def _build_optional_element(
        self, tag: str, value: Any, attributes: dict[str, str] | None = None
    ) -> str:
        ...
        if attributes:
            attr_str = " " + " ".join(
                [f'{k}="{self._escape_xml(v)}"' for k, v in attributes.items()]
            )

        escaped_value = self._escape_xml(str(value))
        return f"<{tag}{attr_str}>{escaped_value}</{tag}>"

    def _build_required_element(
        self, tag: str, value: Any, default: str = "", attributes: dict[str, str] | None = None
    ) -> str:

...
class PractitionerXMLBuilder(BaseXMLBuilder):
    ...
    def build(self, practitioner_data: dict[str, Any]) -> str:
        ...
        if not practitioner_data:
            log.warning(
                "PractitionerXMLBuilder: No practitioner data available, creating minimal Provider section"
            )
            return self._build_minimal_provider()

        # Extract practitioner info
        role = practitioner_data.get("role", "")
    ...
    def _build_license_section(self, practitioner_data: dict[str, Any]) -> str:
        ...
        log.info(
            f"PractitionerXMLBuilder: Building license section with license_number: {license_number}, license_type: {license_type}, license_state: {license_state}"
        )

...
class ReportRequestXMLBuilder(BaseXMLBuilder):
    ...
    def build(self, data: dict[str, Any]) -> str:
        ...
        xml_request = self._build_report_request(
            practitioner_data, organization_data, practice_location_data
        )

        log.info(f"ReportRequest XML built: {len(xml_request)} characters")
        return xml_request
    ...
    def _build_report_request(
        ...
        practice_location_data: dict[str, Any],
    ) -> str:
        ...
        log.info("ReportRequestXMLBuilder: Practitioner data:")
        ...
        location_name = practice_location_data.get("name")
        location_dea = practice_location_data.get("dea")
        location_npi = practice_location_data.get("npi")

        # Handle nested address data
        address_data = practice_location_data.get("address", {})
        if isinstance(address_data, dict):
            location_state = address_data.get("state")
        ...
        log.info("ReportRequestXMLBuilder: Location data:")
